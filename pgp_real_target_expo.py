...
import time
import functools
from utils import ParameterType, eval, train, LoggerForSacred, gngtrain
...
from prune_utils import prune_fc_like, get_prune_index_ratio, create_conv_tensor, \
    ...
    in_place_load_state_dict, create_new_bn, get_prune_index_target_with_reset
...
def rpgp_target(epochs, target_prune_rate, remove_ratio, test_loader,
    ...
    for epoch in range(1, epochs + 1):
        ...
        start = time.clock()
        total_loss, prune_index_dict = tcriterion_func(model, model_adapter, optimizer, cuda, train_loader, is_break)
        end = time.clock()
        acc = eval(model, cuda, test_loader,is_break)

        if logger is not None:
            logger.log_scalar("ppgp_target_{}_epoch_time".format(logger_id), start - end, epoch)
            logger.log_scalar("ppgp_target_{}_training_loss".format(logger_id), total_loss, epoch)
            logger.log_scalar("ppgp_target_{}_before_target_val_acc".format(logger_id), acc, epoch)
                ...
                if param_type == ParameterType.CNN_WEIGHTS:
                    ...
                    if tensor_index == 1:
                        ...
                        if downsample_cnn is not None:
                            ...
                            num_remain_target_prune = original_c[d_name] # No prune for downsampling test
                            ...
                            if reset_index is not None:
                                removed_filters_total_epoch += original_out_channels - keep_index.shape[0] - len(reset_index)
                                reset_filters_total_epoch += len(reset_index)
                            else:
                                removed_filters_total_epoch += original_out_channels - keep_index.shape[0]
                            parameters_hard_removed_per_epoch += (original_out_channels - keep_index.shape[0]) * \
        ...
        acc = eval(model, cuda, test_loader,is_break)
        if logger is not None:
            logger.log_scalar("rpgp_target_{}_after_target_val_acc".format(logger_id), acc, epoch)
            logger.log_scalar("rpgp_target_{}_number of filter removed".format(logger_id), removed_filters_total + reset_filters_total_epoch, epoch)
            logger.log_scalar("rpgp_target_{}_acc_number of filter removed".format(logger_id), acc, removed_filters_total + reset_filters_total_epoch)
            logger.log_scalar("rpgp_target_{}_acc_number of parameters removed".format(logger_id), acc,
    ...
    if final_fn[0]:
        ...
        for epoch in range(epochs + 1, epochs + final_fn[0] + 1):
            ...
            if logger is not None:
                logger.log_scalar("rpgp_{}_epoch_time".format(logger_id), start - end, epoch)
                logger.log_scalar("rpgp_{}_training_loss".format(logger_id), total_loss, epoch)
                logger.log_scalar("rpgp_{}_before_target_val_acc".format(logger_id), acc, epoch)
...
def main():
    ...
    logger = None
    rpgp_target(epochs, t_prune_rate, remove_ratio, testloader, gngtrain,
                cuda=cuda, model=model, train_loader=trainloader, train_ratio=1,
                initializer_fn=zero_initializer, optimizer=optimizer, logger=None, model_adapter=ResNetAdapter(),
                is_expo=True, is_break=False,scheduler=scheduler, final_fn=[20, 0.01]) #final_fn = [epochs, finetune learning rate]
